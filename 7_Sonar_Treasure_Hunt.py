...
def drawBoard(board):
    ...
    hline = ' ' # initial space for the numbers down the left side ofthe board
...
To play, enter the coordinates of the point in the ocean you wish to dorp a
...
chests up to a distance of 9. If all chests are out of range, the point will be marked with 0

If a device is directly dropped on a treasure chest, you have discovered
the location of the chest, and it will be collected. The sonar device will remain there.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        print('You have %s sonar device%s left. %s treasure chests%s remaining.' % (sonarDevices, extraSsonar, len(theChests), extraSchest))

        x, y = enterPlayerMove()
        previousMoves.append([x, y]) # we must trach all moves so that sonar devices can be updated.
